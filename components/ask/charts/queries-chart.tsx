...
const groupQueriesByDate = (queries: Array<{ timestamp: string; isConsensusReached: boolean }>): QueryData[] => {
  const dateMap = new Map<string, { total: number; consensusReached: number; consensusFailed: number }>();
  
  queries.forEach(query => {
    const date = new Date(query.timestamp).toISOString().split('T')[0];
    const current = dateMap.get(date) || { total: 0, consensusReached: 0, consensusFailed: 0 };
...
export default function QueriesChart() {
  ...
  React.useEffect(() => {
    const fetchQueryData = async () => {
      try {
        ...
        // Get recent queries (last 100 for charting)
        const response = await fetch('/api/vote-history?limit=100&offset=0');
  ...
  const [activeChart, setActiveChart] =
    React.useState<keyof typeof chartConfig>("totalQueries");

  const total = React.useMemo(
    () => ({
      totalQueries: chartData.reduce((acc, curr) => acc + curr.totalQueries, 0),
      consensusReached: chartData.reduce((acc, curr) => acc + curr.consensusReached, 0),
      consensusFailed: chartData.reduce((acc, curr) => acc + curr.consensusFailed, 0),
    }),
    [chartData]
  );
      ...
      <Card>
        <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row">
          ...
          <CardTitle>Testnet Query Activity</CardTitle>
            <CardDescription>
              Total queries processed: {totalCount.toLocaleString()}
            </CardDescription>
          </div>
          <div className="flex">
