...
export function ValidatorList(props: ValidatorListProps) {
  ...
  const validValidators = Array.isArray(props.validators) ? props.validators : [];
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
      <h3 className="text-lg font-medium mb-4">Validators ({validValidators.length})</h3>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {validValidators.length > 0 ? (
              validValidators.map((validator, index) => (
                <tr key={validator.id} className={index % 2 === 0 ? "bg-gray-50 dark:bg-gray-900/50" : ""}>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">{validator.provider}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm">
                    {props.currentLeaderIndex !== undefined && index === props.currentLeaderIndex ? (
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300 max-w-md">
                    {validator.lastRationale || 
                     <span className="text-gray-400 dark:text-gray-500 italic">No rationale available</span>}
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={5} className="px-4 py-3 text-sm text-center text-gray-500 dark:text-gray-400">
