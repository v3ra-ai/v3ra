 ...
 */
export function ValidatorHealthCheck({ onHealthIssue, className = '' }: HealthCheckProps) {
  ...
  if (!health) {
    return (
      <div className={`flex items-center space-x-2 p-3 bg-gray-800 rounded-md ${className}`}>
        <div className="animate-pulse h-3 w-3 rounded-full bg-yellow-500"></div>
        <span className="text-gray-400 text-sm">Checking validator health...</span>
      </div>
    );
  }
    ...
    <div className={`overflow-hidden transition-all duration-300 ${className}`} data-testid="validator-health-check">
      ...
      >
        <div className="flex items-center space-x-2">
          <div className={`h-3 w-3 rounded-full ${statusColors[health.status]}`}></div>
          <span className={`text-sm ${health.status === 'healthy' ? 'text-gray-300' : 'text-white'}`}>
            {health.message}
          </span>
        </div>
