...
import { OpenAIValidator, AnthropicValidator, GeminiValidator } from './providers';
...
class ElizaValidator extends OpenAIValidator {
  constructor() {
    ...
    this.description = 'A specialized agent running on Eliza OS with a Data Scientist character profile, providing insights focused on statistical validity and data integrity.';
...
export function initializeValidators() {
  ...
  validatorRegistry.addValidator(new OpenAIValidator({
    modelName: 'gpt-4o',
    name: 'GPT-4o Validator'
  }));
  
  validatorRegistry.addValidator(new AnthropicValidator({
  ...
  validatorRegistry.addValidator(new GeminiValidator({
