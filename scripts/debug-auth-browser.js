...
(() => {
  ...
  var noop2, WORKER_SCRIPT, RealtimeClient;
  ...
  var Logger = class _Logger {
    ...
    log(level, message, data) {
      ...
      switch (level) {
        case "debug":
          console.log(formattedMessage, data !== void 0 ? data : "");
          break;
        case "info":
          console.info(formattedMessage, data !== void 0 ? data : "");
          break;
        case "warn":
          console.warn(formattedMessage, data !== void 0 ? data : "");
          break;
        case "error":
          console.error(formattedMessage, data !== void 0 ? data : "");
          break;
      }
    }
    debug(message, data) {
  ...
  var apiLogger = new Logger({ namespace: "api" });
  var pointsLogger = new Logger({ namespace: "points" });

  // lib/supabase-client.ts
  var logger2 = createLogger("supabase-client");
  ...
  var cookieStorage = {
    ...
    setItem(key, value) {
      ...
      const cookieOptions = [
        ...
        "max-age=31536000",
        "SameSite=Lax",
        // Allow cookies on navigation from external sites
        isProduction ? "Secure" : ""
      ].filter(Boolean).join("; ");
  ...
  // scripts/debug-auth-issue.ts
  async function debugAuthIssue() {
    console.log("\u{1F50D} Debugging Authentication Issue\n");
    ...
    if (error) {
      ...
      console.log("   Expires at:", new Date(session.expires_at * 1e3).toISOString());
    } else {
    ...
    if (typeof document !== "undefined") {
      ...
      const authCookies = cookies.filter(
        (c) => c.includes("sb-") || c.includes("supabase") || c.includes("csrf")
      );
    ...
    try {
      ...
      if (pointsResponse.ok) {
        ...
        const error2 = await pointsResponse.text();
        console.log("   Error:", error2);
      }
    } catch (error2) {
      console.log("\u274C API test error:", error2);
    }
    console.log("\n\u{1F4CB} Troubleshooting steps:");
